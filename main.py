...
# Use TELEGRAM_BOT_TOKEN from config.py
...
┏┳┓       •    ┓┳┓┏┓┏┳┓
 ...
 ┻ ┗ ┛ ┛┗┗┗┛┗┗┻┗┻┛┗┛ ┻ 
 Status : Active 
   Made By TG: @ANONYMOUS_AMIT 
    """
    # Send the logo text
    update.message.reply_text("```" + logo_text + "```", parse_mode='MarkdownV2')
...
def ls_command(update: Update, context: CallbackContext) -> None:
    files = os.listdir('.')
    update.message.reply_text("```Files in current directory:\n" + '\n'.join(files) + "```", parse_mode='MarkdownV2')

# Function to handle the /pwd command
def pwd_command(update: Update, context: CallbackContext) -> None:
    cwd = os.getcwd()
    update.message.reply_text("```Current directory: " + cwd + "```", parse_mode='MarkdownV2')
...
def cd_command(update: Update, context: CallbackContext) -> None:
    ...
    if len(args) == 1:
        ...
        try:
            os.chdir(directory)
            update.message.reply_text("```Changed directory to: " + directory + "```", parse_mode='MarkdownV2')
        except FileNotFoundError:
            update.message.reply_text("```Directory not found: " + directory + "```", parse_mode='MarkdownV2')
    else:
    ...
    if len(args) == 1:
        ...
        try:
            with open(file_path, 'r') as file:
                content = file.read()
                update.message.reply_text("```Content of " + file_path + ":\n" + content + "```", parse_mode='MarkdownV2')
        except FileNotFoundError:
            update.message.reply_text("```File not found: " + file_path + "```", parse_mode='MarkdownV2')
    else:
...
def mkdir_command(update: Update, context: CallbackContext) -> None:
    ...
    if len(args) == 1:
        ...
        try:
            os.mkdir(directory)
            update.message.reply_text("```Directory created: " + directory + "```", parse_mode='MarkdownV2')
        except FileExistsError:
            update.message.reply_text("```Directory already exists: " + directory + "```", parse_mode='MarkdownV2')
    else:
...
def git_clone_command(update: Update, context: CallbackContext) -> None:
    ...
    if len(args) == 1:
        ...
        try:
            ...
            update.message.reply_text("```Cloned repository successfully: " + repo_url + "```", parse_mode='MarkdownV2')
        except subprocess.CalledProcessError:
            update.message.reply_text("```Failed to clone repository: " + repo_url + "```", parse_mode='MarkdownV2')
    else:
...
def run_python_command(update: Update, context: CallbackContext) -> None:
    ...
    if len(args) == 1:
        ...
        try:
            ...
            update.message.reply_text("```Failed to run Python script: " + script_path + "```", parse_mode='MarkdownV2')
    else:
...
def run_pip_command(update: Update, context: CallbackContext) -> None:
    ...
    if len(args) >= 1:
        ...
        try:
            ...
            update.message.reply_text("```Successfully installed packages: " + packages + "```", parse_mode='MarkdownV2')
        except subprocess.CalledProcessError:
            update.message.reply_text("```Failed to install packages: " + packages + "```", parse_mode='MarkdownV2')
    else:
...
def shell_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            update.message.reply_text("```" + result.stdout + "```", parse_mode='MarkdownV2')
        else:
            update.message.reply_text("```Error: " + result.stderr + "```", parse_mode='MarkdownV2')
    except Exception as e:
        update.message.reply_text("```An error occurred: " + str(e) + "```", parse_mode='MarkdownV2')
